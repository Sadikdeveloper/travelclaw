...
| **A pace per model**             | The desk runs the strongest model it can run — nobody picks one, guests least of all — and chat turns are limited **per model and per tier** in ten-minute windows. A bigger model is rationed tighter than a small one, and signing in buys several times a guest's allowance on it. The numbers live in `apps/api/src/models/model-catalog.ts`; `GET /api/models` publishes them and the model in use. |

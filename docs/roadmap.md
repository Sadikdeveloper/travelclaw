...
This is an independent travel desk inspired by the OpenClaw monorepo idea (one gateway, workspace markdown, skills, channels, a control UI). It is not a fork and not affiliated with OpenClaw.
...
3. `@travelclaw/agent-core` skills and the turn loop, with tests and no HTTP
...
- [x] Shared contracts
- [x] Agent core and bundled travel skills
- [x] Gateway API
- [x] Control UI
- [x] CI and Docker
...
4. **Model-selected tools.** Skills currently run from a deterministic router, then the model narrates. Let an OpenAI-compatible model request a skill, and keep the router as fallback.
...
9. **Flight and stay search skill** behind an explicit provider key. Never imply a booking succeeded.

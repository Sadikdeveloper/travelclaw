...
- Chat turns are paced **per model and per tier**, in ten-minute windows. The model is the desk's choice, not the traveler's: a guest cannot select one, a signed-in account does not get to either, and a request that names one is refused. The desk runs the strongest model it can actually run, and every model carries its own guest and account allowance in `apps/api/src/models/model-catalog.ts` — so a bigger, costlier model is rationed tighter than a small one for everybody, and a signed-in account's allowance is several times a guest's on the model both of them are using. That is what signing in buys. Guests are additionally capped per IP across all models, so a guest that hits a model's pace and mints a fresh guest to dodge it still hits the address-wide cap. `GET /api/models` publishes the catalog and the limit in force.
